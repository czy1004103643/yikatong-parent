...
layui.define('jquery', function(exports){
  ...
  Tree.prototype.tree = function(elem, children) {
    ...
    layui.each(nodes, function(index, item){
      ...
      var li = $(['<li '+ (item.spread ? 'data-spread="'+ item.spread +'"' : '') +'>'
        ...
        ,function() {
        	// 如果定义了click则不触发href
          return '<a href="'+ (typeof(options.click) ==='undefined' ? item.href : 'javascript:;') +'" '+ (
  ...
  //暴露接口
  var treeOptions = {};
  exports('tree', function(options, reload){
	// 重新加载当前树
	if(reload) {
		options = treeOptions[options];
	} else {
		var key = options.elem.replace('#', '');
		treeOptions[key] = options;
	}
