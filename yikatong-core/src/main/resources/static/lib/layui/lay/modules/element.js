      ...
      $('.layui-tab').each(function(){
        ...
        if(othis.attr('lay-allowClose')){
          title.find('li').each(function(){
            var li = $(this);
            // 判断当前tab是否锁定
            if(li.hasClass('lock')) return true;
            if(!li.find('.'+CLOSE)[0]){
  ...
  Element.prototype.init = function(type, filter){
    var that = this, elemFilter = function(){
      ...
      ,nav: function(){
        ...
        // tab 操作
        var tabOperate = $('.mt-tab-operate'), TITLE = '.layui-tab-title li', CONTENT = '.layui-tab-content .layui-tab-item',
        	MENUTREE = '.layui-nav-tree .layui-this';
        tabOperate.on('mouseenter', function() {
        	$(this).find('.layui-anim-upbit').show();
        	// unbind 移除事件重复绑定问题
        	$('.mt-tab-tool-body').find('li').unbind('click').on('click', function(e) {
        		if($(this).data('operate') == 'closeAll') {
        			$(TITLE).each(function(index, data) {
        				if(!$(data).hasClass('lock')) {
        					$(data).remove();
        				} else {
        					$(data).addClass('layui-this');
        				}
        			})
        			...
        			$(MENUTREE).removeClass('layui-this');
        		} else if($(this).data('operate') == 'closeOther') {
        			$(TITLE).each(function(index, data) {
        				if(!$(data).hasClass('lock') && !$(data).hasClass('layui-this')) {
        					$(data).remove();
        				}
        			})
        			$(CONTENT).each(function(index, data) {
        				if(!$(data).hasClass('lock') && !$(data).hasClass('layui-show')) {
        					$(data).remove();
        				}
        			})
        		} else if($(this).data('operate') === 'refresh') {
        			if($(TITLE).length > 1) {
        				var href = $(MENUTREE).attr('href');
        				$('.layui-tab-content .layui-show').load(href)
        			}
        		}
        	})
        }).on('mouseleave', function() {
        	$(this).find('.layui-anim-upbit').hide();
        })
